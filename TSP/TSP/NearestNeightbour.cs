...
namespace TSP
{
    internal class NearestNeightbour : Solver
    {
        State state;
        City currentCity;
        List<City> route = new List<City>();

        public NearestNeightbour(State state)
        {
            this.state = state;
            Solve(state);
        }
        ...
        void Solve(State state)
        {
            ...
            foreach (var city in route) 
            {
                ...
                Console.WriteLine($"Position: {city.Position.X};{city.Position.Y}");
                Console.WriteLine("------------------");
            }
        ...
        Operator GetOperator()
        {
            ...
            {
                if(op == null || op.GetDistance() > Operators[i].GetDistance())
                {
                    op = Operators[i];
                }
            }
